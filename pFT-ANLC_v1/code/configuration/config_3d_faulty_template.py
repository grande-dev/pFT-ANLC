...
A function collecting the parameters of the training of the 3-dimensional system:
template.
...
def set_params():
    ...
    campaign_params = {
        ...
        'campaign_run': 400,   # number of the run:
        ...
        'max_loop_number': 1,  # number of loops per run (default = 1, >1 means that the weights will be re-initialised).
                               # default value = 1.
        'max_iters': 10000,     # number of maximum learning iterations per run.
        'system_name': "3d_template",  # name of the systems to be controlled (only used in the statistics file, pick any name).
        'x_star': torch.tensor([0.0, 0.0, 0.0]),  # x*: target equilibrium point.
    }    
    ...
    learner_params = {
        ...
        'learning_rate_c': 0.01,     # learning rate control ANN (lambda).
    ...
    control_params = {
        ...
        'size_ctrl_layers': [30, 4],      # CAVEAT: the last entry is the number of control actions! In this case the system has 2 actuators.
        'ctrl_bias': [True, False],
        'ctrl_activations': ['tanh', 'linear'],
        'use_saturation': False,          # use saturations in the control law.
        'ctrl_sat': [11.4, 20.2],         # actuator saturation values:
    ...
    falsifier_params = {
        ...
        'gamma_overbar': 0.8,   # CLF domain upper boundary.
    ...
    dyn_sys_params = {
        ...
        'h4': 1.,  # nominal health status of actuator 4
    }

    # Faulty cases
    params_faults = {
        'phi_i': [[1, 1, 1, 1], [1, 0, 1, 1]],
    }
