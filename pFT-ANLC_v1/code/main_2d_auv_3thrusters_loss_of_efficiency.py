...
The main file to run a simulation campaign for the training of a 2-dimensional 
AUV with 3 thrusters system:
- affected by binary faults;
- affected by the loss of efficiency of some actuators.
...
from cegis_ftc_efficiency_loss import cegis
import utilities.saving_log as saving_log
import utilities.saving_stat as saving_stat
from utilities.nn import Net
from utilities.models import AUV2D_FaultyEfficiencyLoss as UsedModel
import configuration.config_2d_auv_faulty_efficiency_loss as config_file
import closed_loop_testing.cl_2d_auv_faulty_efficiency_loss as cl
import systems_dynamics.dynamics_2d_auv_faulty_efficiency_loss as dynamic_sys
...
for i_loop in range(parameters['tot_runs']):
    ...
    parameters, exit_info = cegis(parameters, seed_,
                                  Net,
                                  UsedModel().vars_, UsedModel().eff_, UsedModel.f_torch, UsedModel.f_symb,
                                  config,
