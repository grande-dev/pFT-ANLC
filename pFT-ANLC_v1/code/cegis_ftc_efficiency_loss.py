...
from utilities.falsifier import augm_falsifier, disc_falsifier_fault, falsifier_SMT, disc_falsifier_fault_efficiency_loss, falsifier_SMT_efficiency_loss, compute_symbolic
...
def cegis(parameters, seed_,
    ...
    # generate dataset
    # x_var = torch.Tensor(parameters['N'], parameters['n_input']).uniform_(-parameters['gamma_overbar'], parameters['gamma_overbar'])
    # x_eff = torch.Tensor(parameters['N'], parameters['n_input_eff']).uniform_(0., 1.)
    # x = torch.cat((x_var, x_eff),1)

    x_var = torch.Tensor(parameters['N'], parameters['n_input']).uniform_(-parameters['gamma_overbar'], parameters['gamma_overbar'])
    x_eff = torch.Tensor(parameters['N'], parameters['n_input_eff']).uniform_(parameters['eff_low'], parameters['eff_high'])
    x = torch.cat((x_var, x_eff),1)
    ...
    while out_iters < parameters['max_loop_number'] and not found_lyap_f and not to_fals:
        ...
        V_inter_hist = []
        lie_der_inter_hist = []
        lie_der_inter_hist_tot = []  # unused but used for consistency with FTC file
        u_inter_hist = []
        ...
        while i_epoch < parameters['max_iters'] and not found_lyap_f and not to_fals:
            ...
            for j_fault in range(len(parameters['phi_i'])):
                ...
                V_data, Lie_V_data, Circle_Tuning_data, u_data = \
                    model.forward_eff_loss(x_var, x_eff, f_torch, parameters)
            ...
            x_0_var = torch.zeros([1, parameters['n_input']]) 
            x_0_eff = torch.zeros([1, parameters['n_input_eff']]) 
            V0, Lie_V0, Circle_Tuning_0, u0 = model.forward_eff_loss(x_0_var, x_0_eff, f_torch, parameters)
            ...
            (Lyap_risk_ELR.item() < parameters['tau_overbar'] if parameters['enforce_CLF_shape'] else True):
                ...
                for j_fault in range(len(parameters['phi_i'])):
                    ...
                    for k_ctr in range(parameters['size_ctrl_layers'][-1]):
                        ...
                        str_h = "h" + str(k_ctr+1)
                        parameters[str_h] = parameters['phi_i'][j_fault][k_ctr]
                        # logging.debug("parameters[{str_h}] = ({parameters[str_h]})")

                    
                    x, u_learn, V_learn, lie_derivative_of_V, f_out_sym, disc_viol_found =\
                        disc_falsifier_fault_efficiency_loss(parameters, vars_, eff_, model, f_symb,
                ...
                if sum_df_violations == 0:
                    ...
                    for j_fault in range(len(parameters['phi_i'])):
                        ce_smt_found = 0
                        x, lie_derivative_of_V, f_out_sym, found_lyap_f, to_fals, ce_smt_found =\
                            falsifier_SMT_efficiency_loss(parameters, vars_, eff_, model, f_symb,
                                                        parameters['epsilon'], parameters['gamma_overbar'], 
                                                        config, x)
                ...
                # extracting x dataset components
                x_var = x[:, 0:parameters['n_input']]
                x_eff = x[:, parameters['n_input']:]
