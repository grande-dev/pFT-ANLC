...
def closed_loop_system(samples_number, model, 
    ...
    dynamic_fault_test = True  # inject the fault mid-simulation
    act_faulty_test = randrange(2)+1  # either 1st or 2nd motor not working
    parameters['h1'] = 1.0
    parameters['h2'] = 1.0
    ...
    for iiter in range(samples_number):
        if (iiter == 0):
            ...
            # injecting faults at half of the simulation time
            ...
            if fault_occurred:
                # The faults has occurred already
                u_test.detach()[0,act_faulty_test-1] = 0.0
    ...
    if fault_occurred == True:
        plt.scatter(x_axis_scale[int(time_fault)], x_test_hist[int(time_fault), 0], s=100, c='red', label=f'$F_{act_faulty_test}$ - fault injected')
    plt.xlabel("Time [s]")
    ...
    if fault_occurred == True:
        plt.scatter(x_axis_scale[int(time_fault)], x_test_hist[int(time_fault), 1], s=100, c='red', label=f'$F_{act_faulty_test}$ - fault injected')
    plt.xlabel("Time [s]")
