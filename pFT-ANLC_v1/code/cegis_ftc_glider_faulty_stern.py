...
def cegis(parameters, seed_,
    ...
    while out_iters < parameters['max_loop_number'] and not found_lyap_f and not to_fals:
        ...
        assert model.size_ctrl_layers[-1] == len(parameters['ctrl_sat'])

        i_epoch = 0  # epochs counter

        V_inter_hist = []
        lie_der_inter_hist = []
        lie_der_inter_hist_tot = [[] for x in range(len(parameters['phi_i']))]
        u_inter_hist = []
        ...
        while i_epoch < parameters['max_iters'] and not found_lyap_f and not to_fals:
            ...
            for j_fault in range(len(parameters['phi_i'])):
                ...
                for k_ctr in range(parameters['size_ctrl_layers'][-1]):
                    ...
                    # Injecting faulty stern
                    # injecting the jammed actuator fault
                    if parameters['h2'] == 0.:
                        parameters['off_dp'] = parameters['off_dp_after_fault']
                    else:
                        parameters['off_dp'] = parameters['off_dp_before_fault']

                    #print(parameters[str_h])
                ...
                #print(f"model #{j_fault} = {torch.sum(F.relu(Lie_V_data))}")
            ...
            (Lyap_risk_ELR.item() < parameters['tau_overbar'] if parameters['enforce_CLF_shape'] else True):
                ...
                for j_fault in range(len(parameters['phi_i'])):
                    ...
                    logging.debug(f"u_learn[1]={u_learn[1]}")
                    # logging.debug(f"h1 = {parameters['h1']}")
                    # logging.debug(f"h2 = {parameters['h2']}")
                    # logging.debug(f"h3 = {parameters['h3']}")
                ...
                if sum_df_violations == 0:
                    ...
                    for j_fault in range(len(parameters['phi_i'])):
                        ...
                        ## BIG BUG: report the next lines 'until here' in cegic_ftc
